...
router.post('/register', (req, res) => {
  let { username, password, email, avatar, role } = req.body;
  User.findOne({ email }).then(
    user => {
      ...
      const newUser = new User({ username, password, email, avatar, role });
            ...
            .then(
              isMatch => {
                if (isMatch) {
                  ...
                  // 在登录验证成功后返回的内容
                  const token = jwt.sign(user.toObject(), privateKey, { expiresIn: '1h' });
